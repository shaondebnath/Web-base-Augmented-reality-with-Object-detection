...
module.exports = function(socket, robot) {
  ...
  socket.on('click', params => {
    console.log("button click server")
...
//shaon
    //socket.emit('resolutionData',{srcWidth: 400, srcHeight: 300 });
  ...
  socket.on('reqMousePos', ()=>{console.log("mousePos Req")
      socket.emit('getMousePos',{
          getMousePosX: robot.getMousePos().x,
          getMousePosY: robot.getMousePos().y
